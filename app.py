...
if st.button("🔮 تشغيل التنبؤ"):
    ...
    def clean_table(data_subset):
        ...
        # 🔹 تطبيق التحويل للعربية هنا
        res["اليوم"] = res.index.day_name().map(days_map)
        res["نوع البيانات"] = np.where(res.index <= last_real_date, "Historical", "Forecasted")
